...
if __name__ == '__main__':
    ...
    #load train data
    train_set = VeRIWildDataset(train_transform, args.train_image_dir, 5024, TRAIN_PIC_NUM) 
    ...
    for epoch in range(epochs):
        ...
        for bch, data in enumerate(tqdm(train_loader, dynamic_ncols=True, desc=f'epoch {epoch+1}/{epochs}')):
            ...
            #         length += feature[i][j] * feature[i][j]
            #     length = math.sqrt(length)
            #     feature[i] = feature[i] / length
            #     del length
            ...
            #compute loss
            optimizer.zero_grad()
            Ada_SP_loss = SP_loss(feature)(feature)
            loss = Ada_SP_loss
            ...
            del img, feature, Ada_SP_loss, loss
            torch.cuda.empty_cache()
